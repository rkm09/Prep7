...
//    order by price, shop & movie; do not return 0
    private static final Comparator<Node> CMP =
            Comparator.comparingInt((Node a) -> a.price).thenComparingInt(a -> a.shop).thenComparingInt(a -> a.movie);
...
//    return up to 5 shops with this movie; cheapest -> shop
    public List<Integer> search(int movie) {
        List<Integer> movies = new ArrayList<>();
        TreeSet<Node> set =  availableByMovie.get(movie);
        if(set == null || set.isEmpty()) return movies;
        Iterator<Node> it = set.iterator();
        for(int i = 0 ; i < 5 && it.hasNext() ; i++) {
            movies.add(it.next().shop);
        }
        return movies;
    }
...
//    move (shop, movie) from available -> rented
    public void rent(int shop, int movie) {
        long key = key(shop, movie);
        Node node = byPair.get(key);
        if(node == null) return;
        TreeSet<Node> set = availableByMovie.get(movie);
...
//    return up to 5 rented copies; [shop, movie] cheapest -> shop asc -> movie asc
    public List<List<Integer>> report() {
        List<List<Integer>> repList = new ArrayList<>(5);
        Iterator<Node> it = rentedSet.iterator();
        for(int i = 0 ; i < 5 && it.hasNext() ; i++) {
            Node node = it.next();
            repList.add(Arrays.asList(node.shop, node.movie));
        }
        return repList;
    }
}
